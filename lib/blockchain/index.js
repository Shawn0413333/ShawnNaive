...
class Blockchain {
    ...
    //COMP4142(1) - registration of student ID and public key
    regTransaction(studentId, publicKey){
        let registTransaction = Transaction.fromJson({
        ...
        this.addTransaction(registTransaction);

        return registTransaction;
    }
    ...
    // COMP4142(3a) check signature is correct
    checkSignature(attendanceCert){
        let message = {
            timestamp: attendanceCert.timestamp,
            studentId: attendanceCert.studentId,
            course: attendanceCert.course,
        }
        let messageHash = CryptoUtil.hash(message);
        return CryptoEdDSAUtil.verifySignature(attendanceCert.address, attendanceCert.signature, messageHash);;
    }
    
    // COMP4142(3b) generate transaction - type: attandance
    attendenceTransaction(certificate){
        if (!this.checkSignature(certificate)){
            console.log("Signature is invalid. Attendance taking fails. ")
            return;
        }
        certificate.amount = 0;
        let attendTransaction = Transaction.fromJson({
            id: CryptoUtil.randomId(64),
            hash: null,
            type: 'attendance',
            data: {
        ...
        this.addTransaction(attendTransaction);

        return attendTransaction;
    }
