...
class Miner {
    ...
    static generateNextBlock(rewardAddress, blockchain) {
        ...
        const previousHash = previousBlock.hash;
        const timestamp = new Date().getTime() / 1000;
        const nonce = 0; // COMP4142
        const blocks = blockchain.getAllBlocks();

        const transactions = JSON.parse(JSON.stringify(blockchain.transactions)); //COMP4142 Lab2


        // const candidateTransactions = JSON.parse(JSON.stringify(blockchain.transactions)); //COMP4142 Lab2
    ...
    mine(publicKey) {
        ...
        let baseBlock = Miner.generateNextBlock(publicKey, this.blockchain);
        return baseBlock;
    }
    ...
    static proveWorkFor(block, difficulty) {
        ...
        do {
            ...
            // //testing
            // console.log("5. the current block timestamp: " + block.timestamp + "; nonce: " + block.nonce +"; hash: " + block.nonce +"; Diffi: " + blockDifficulty +" >=? " + difficulty )
