...
class Operator {
    ...
    // COMP4142(2) - generate attendance cert
    generateAttendanceCert(studentId, eventId) {
        let cert = {
    ...
    // COMP4142(2) - sign attendance cert
    signAttendanceCert(attendanceCert, addressId, secretKey){
        if (secretKey == null) throw new ArgumentError(`Secret key cannot be null`);

        // Encryption of attendance cert hash using inputted private key
        let messageHash = CryptoUtil.hash(attendanceCert);
        attendanceCert.signature = CryptoEdDSAUtil.signHash(CryptoEdDSAUtil.generateKeyPairFromSecret(secretKey), messageHash);

        attendanceCert.address = addressId;
