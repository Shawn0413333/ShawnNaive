...
class HttpServer {
    constructor(node, blockchain, operator, miner) {
        ...
        // COMP4142(1) - Registration of student ID and public key to the blockchain
        this.app.post('/blockchain/registration', (req, res) => {
            let studentId = req.body.studentId;
            let publicKey = req.body.publicKey;
            if (studentId == null) throw new HTTPError(401, 'Student ID is missing.');
            if (publicKey == null) throw new HTTPError(401, 'Public key is missing.');

            try {
                let registration = blockchain.regTransaction(studentId, publicKey);
                res.status(201).send(registration);
            } catch (ex) {
                if (ex instanceof TransactionAssertionError) throw new HTTPError(400, ex.message, registration, ex);
                else throw ex;
            }
        });

        // COMP4142(3) - add transaction with "attendance" type into blockchain
        this.app.post('/blockchain/attendance', (req, res) => {
            let certificate = req.body;
            if (certificate == null) throw new HTTPError(401, 'Certificate is missing.');

            try {
                let attendance = blockchain.attendenceTransaction(certificate);
                res.status(201).send(attendance);
            } catch (ex) {
                if (ex instanceof TransactionAssertionError) throw new HTTPError(400, ex.message, attendance, ex);
                else throw ex;
            }
        });
        ...
        // COMP4142(2a) - Generate of attendance cert
        this.app.post('/operator/generateAttendanceCert', (req, res) => {
            let studentId = req.body.studentId;
            let eventId = req.body.eventId;
            if (studentId == null) throw new HTTPError(401, 'Student ID is missing.');
            if (eventId == null) throw new HTTPError(401, 'Event ID is missing.');

            let cert = operator.generateAttendanceCert(studentId, eventId);
            res.status(201).send(cert);
        });

        // COMP4142(2b) - Signing on attendance cert
        this.app.post('/operator/signAttendanceCert', (req, res) => {
            let attendanceCert = req.body.attendanceCert;
            let addressId = req.body.addressId;
            let secretKey = req.body.secretKey;
            if (attendanceCert == null) throw new HTTPError(401, 'Attendance cert is missing.');
            if (addressId == null) throw new HTTPError(401, 'Address ID is missing.');
            if (secretKey == null) throw new HTTPError(401, 'Secret key is missing.');

            let signCert = operator.signAttendanceCert(attendanceCert, addressId, secretKey);
            res.status(201).send(signCert);
        });
